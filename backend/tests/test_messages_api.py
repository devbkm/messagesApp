...
def test_create_trims_surrounding_whitespace(client: TestClient) -> None:
    body = create(client, user_headers(), subject="  Padded  ", text="\n Text \n")
...
def test_subject_of_exactly_40_characters_is_accepted(client: TestClient) -> None:
    body = create(client, user_headers(), subject="s" * 40)
...
def test_invalid_payloads_are_rejected(
    ...
    response = client.post(URL, json=payload, headers=user_headers())
...
def test_validation_errors_do_not_echo_input(client: TestClient) -> None:
    secret = "s" * 41
    response = client.post(URL, json={"subject": secret, "text": "Body"}, headers=user_headers())
...
def test_malformed_bodies_are_rejected(client: TestClient, content: str, content_type: str) -> None:
    response = client.post(
        URL, content=content, headers={**user_headers(), "Content-Type": content_type}
    )
...
def test_get_unknown_message_returns_404(client: TestClient) -> None:
    response = client.get(f"{URL}/{uuid.uuid4()}", headers=user_headers())
...
def test_cannot_read_another_users_message(client: TestClient) -> None:
    owner, intruder = user_headers(), user_headers()
...
@pytest.mark.parametrize("headers", [{}, {"X-User-Id": ""}, {"X-User-Id": "not-a-uuid"}])
def test_requests_without_valid_identity_are_rejected(
...
def test_database_outage_returns_503_without_details(unreachable_db_client: TestClient) -> None:
    response = unreachable_db_client.get(URL, headers=user_headers())
...
def test_database_error_returns_generic_500(
    ...
    response = client.post(URL, json={"subject": "Hi", "text": "Body"}, headers=user_headers())
...
def test_unexpected_error_returns_generic_500(db: Session, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app, raise_server_exceptions=False) as test_client:
        response = test_client.get(URL, headers=user_headers())
...
def test_unsupported_method_uses_error_envelope(client: TestClient) -> None:
    response = client.put(URL, json={}, headers=user_headers())
...
def test_openapi_documents_the_contract(client: TestClient) -> None:
    ...
    assert spec["components"]["securitySchemes"]["UserId"] == {
        "type": "apiKey",
        "in": "header",
        "name": "X-User-Id",
        "description": "Development-only user identification: the caller's user id (UUID).",
    }
    assert paths[URL]["get"]["security"] == [{"UserId": []}]
