...
def test_multiline_text_with_tabs_is_preserved_exactly(client: TestClient) -> None:
    text = "Dear team,\n\n\tPoint one\r\n\tPoint two\n\nBye"
    response = client.post(URL, json={"subject": "Notes", "text": text}, headers=headers())
...
def test_forty_one_emoji_subject_is_rejected(client: TestClient) -> None:
    ...
    response = client.post(URL, json={"subject": "😀" * 41, "text": "Body"}, headers=headers())
...
def test_maximum_length_text_is_accepted(client: TestClient) -> None:
    ...
    response = client.post(URL, json={"subject": "Long", "text": text}, headers=headers())
...
def test_body_without_declared_length_is_rejected(client: TestClient) -> None:
    ...
    response = client.post(
        URL, content=chunks(), headers={**headers(), "Content-Type": "application/json"}
    )
...
def test_equivalent_uuid_spellings_identify_the_same_user(client: TestClient) -> None:
    user_id = uuid.uuid4()
    client.post(URL, json={"subject": "Mine", "text": "Body"}, headers={"X-User-Id": str(user_id)})

    for spelling in (user_id.hex, f"{{{user_id}}}", str(user_id).upper(), f"urn:uuid:{user_id}"):
        items = client.get(URL, headers={"X-User-Id": spelling}).json()["items"]
        assert [item["subject"] for item in items] == ["Mine"]


def test_oversized_identity_header_is_rejected(client: TestClient) -> None:
    response = client.get(URL, headers={"X-User-Id": "x" * 5000})

    assert response.status_code == 401


def test_message_ids_cannot_reach_other_users_data_by_any_route(client: TestClient) -> None:
    owner, intruder = headers(), headers()
...
def test_cors_allows_only_configured_origins() -> None:
    ...
    assert "x-user-id" in allowed.headers["access-control-allow-headers"].lower()
