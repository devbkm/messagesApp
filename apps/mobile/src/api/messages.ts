...
export async function listMessages(): Promise<MessageList> {
  const data = await apiRequest<unknown>(BASE);
  if (!isMessageList(data)) throw invalidResponse();
  return data;
}

export async function getMessage(id: string): Promise<Message> {
  const data = await apiRequest<unknown>(`${BASE}/${encodeURIComponent(id)}`);
