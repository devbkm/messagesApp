...
export function useMessageList() {
  return useQuery({ queryKey: messageKeys.list(), queryFn: listMessages });
}
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // Deleted elsewhere (another device): keep the inbox from showing a ghost row.
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      try {
        ...
        // Already deleted (e.g. on another device): the user's intent is fulfilled.
